...
import gg.moonflower.etched.registry.network.play.ClientboundInvalidEtchUrlPacket;
...
public class EtchingMenu extends AbstractContainerMenu implements UrlMenu {
    ...
    public static final ResourceLocation EMPTY_SLOT_MUSIC_DISC = Etched.id("item/empty_etching_table_slot_music_disc");
    public static final ResourceLocation EMPTY_SLOT_MUSIC_LABEL = Etched.id("item/empty_etching_table_slot_music_label");
    ...
    public EtchingMenu(int id, Inventory inventory, ContainerLevelAccess containerLevelAccess) {
        super(EtchedMenuTypes.ETCHING_MENU, id);
        ...
        this.resultSlot = this.addSlot(new Slot(result, 0, 116, 43) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return false;
            }
    ...
    private void setupResultSlot() {
        ...
        ServerPlayNetworking.send((ServerPlayer) this.player, new ClientboundInvalidEtchUrlPacket(""));
        ...
        if (this.labelIndex.get() >= 0 && this.labelIndex.get() < DiscAppearanceComponent.LabelPattern.values().length) {
            ...
            if (discStack.is(EtchedItems.ETCHED_MUSIC_DISC) || (!discStack.isEmpty() && !labelStack.isEmpty())) {
                ...
                this.currentRequest = CompletableFuture.supplyAsync(() -> {
                    ...
                    if (discStack.is(EtchedItems.ETCHED_MUSIC_DISC)) {
                        resultStack = discStack.copyWithCount(1);
                        resultStack.remove(EtchedComponents.ALBUM);
                    } else {
                        resultStack = new ItemStack(EtchedItems.ETCHED_MUSIC_DISC);
                    }
                    ...
                    if (SoundSourceManager.isValidUrl(this.url)) {
                        try {
                            ...
                            ServerPlayNetworking.send((ServerPlayer) this.player, new ClientboundInvalidEtchUrlPacket(e instanceof CompletionException ? e.getCause().getMessage() : e.getMessage()));
                        ...
                        try {
                            ...
                            ServerPlayNetworking.send((ServerPlayer) this.player, new ClientboundInvalidEtchUrlPacket("Unknown host: " + this.url));
                            throw new CompletionException("Invalid URL", e);
                        } catch (Exception e) {
                            ServerPlayNetworking.send((ServerPlayer) this.player, new ClientboundInvalidEtchUrlPacket(e.getMessage()));
                            throw new CompletionException("Invalid URL", e);
                        }
                    } else {
                        try {
