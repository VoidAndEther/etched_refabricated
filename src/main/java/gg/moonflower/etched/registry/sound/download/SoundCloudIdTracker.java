package gg.moonflower.etched.registry.sound.download;
...
public final class SoundCloudIdTracker {
    ...
    private static String findScriptUrl(Proxy proxy) throws IOException {
        ...
        try {
            ...
            URL uRL = URI.create("https://soundcloud.com").toURL();
    ...
    private static @Nullable String findIdFromScript(String url, Proxy proxy) {
        try {
            ...
            HttpURLConnection httpURLConnection = (HttpURLConnection) URI.create(url).toURL().openConnection(proxy);
            httpURLConnection.setInstanceFollowRedirects(true);
            Map<String, String> map = SoundDownloadSource.getDownloadHeaders();
