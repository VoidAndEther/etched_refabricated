...
public record DiscAppearanceComponent(LabelPattern pattern, int discColor, int labelPrimaryColor, int labelSecondaryColor) {
    ...
    public static int getColor(ItemStack stack, int index) {
        ...
        DiscAppearanceComponent.LabelPattern pattern = discAppearance.pattern();
        if (pattern.isColorable()) {
            if (index == 1) {
                return discAppearance.labelPrimaryColor();
            }
            if (pattern.isComplex() && index == 2) {
    ...
    public enum LabelPattern {
        ...
        LabelPattern(boolean simple) {
            ...
            this.textures = Pair.of(
                    Etched.id("textures/item/" + name + "_label" + (simple ? "" : "_top") + ".png"),
                    Etched.id("textures/item/" + name + "_label" + (simple ? "" : "_bottom") + ".png")
            );
        }
